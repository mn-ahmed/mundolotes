...
class RestApi(http.Controller):
    ...
    def generate_report(self, xml_id, ids):
        ...
        try:
            result, format = request.env.ref(xml_id).with_user(SUPERUSER_ID)._render_qweb_pdf(ids)
            if not result:
                tb = sys.exc_info()
                self_reports['exception'] = odoo.exceptions.UserError('RML is not available at specified location or not enough data to print! %s' % tb)
            ...
            if hasattr(exception, 'name') and hasattr(exception, 'value'):
                self_reports['exception'] = odoo.exceptions.UserError('%s %s' % (tools.ustr(exception.name), tools.ustr(exception.value)))
            else:
                tb = sys.exc_info()
                self_reports['exception'] = odoo.exceptions.UserError('%s %s' % (tools.exception_to_unicode(exception), tb))
            self_reports['state'] = True
